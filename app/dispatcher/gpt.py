...
openai.api_key = settings.OPENAI_API_KEY #os.getenv("OPENAI_API_KEY")
...
################################################################
@router.post("/mes_for_gpt1/")#, response_model=UserResponse)
async def answer_openai_handler1(request: Request):
    data = await request.json()
    text = data['text']
    ...
    #)
    answer=[]
    count=len(response.choices[0].message.content)
    answer.append(response.choices[0]['message']['content'])
    answer.append(count) # +str(count)
    # answer='Ответ HelperGPT:\n' + response.choices[0].text+str(count)
    ...
    #     with open(str(n) + 'image_name.png', 'wb') as handler:
...
#   return {'css_stat': '/css-static/', 'src_stat': '/src-static/', 'img_stat': '/img-static/',}
@router.get("/read_file/")#, response_model=UserResponse)
async def read_file_handler(request: Request):
    #data = await request.json()
...
@router.post("/write_promt/")#, response_model=UserResponse)
async def writePromt_to_fileButton(request: Request):
    data = await request.json()
    text = data['text']
    with open('promt.txt', 'r') as fp:# считываем последнюю стр и опредляем наличие "\n"
        cont = fp.readlines() 
